...
#
# The domains are not here. They are typed into Coolify's own domain field, one per service,
# and the compose file deliberately says nothing about them: a `SERVICE_FQDN_*` declared there
# makes Coolify the owner of the value, which locks the field and rewrites it on every read.
set -euo pipefail
