...
| `NX_MASTER_KEY_FILE` | api, console, worker | The root key, from a file rather than a variable. Accepted in production, unlike `NX_MASTER_KEY`: a variable is readable by anything that can run `docker inspect` or read `/proc/<pid>/environ` (ADR-151). Ignored where `NX_KMS_ENDPOINT` is set. The compose stack makes one on first start. | Required in production unless a key service is configured | Yes |
