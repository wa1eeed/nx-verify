...
> **هذا المستند بالعربية للتركيب الأول.** الدليل الإنجليزي للنشر، ومعه ما يوقف الإطلاق وما يُفحص بعده، في [`guides/deploy-production.md`](guides/deploy-production.md)، وبناء بيئة staging نسخةً منه في [`guides/deploy-staging.md`](guides/deploy-staging.md).
...
من اللوحة: `/operator/verification/integration`، بعد الدخول بحساب من فريق الإدارة. أول زيارة لـ`/operator/login` تطلب رمز النشر `NX_OPERATOR_TOKEN` مرة واحدة لإنشاء حساب المالك، ثم يدخل كل عضو ببريده وكلمة مروره ويضيف المالك بقية الفريق من «الصلاحيات والتدقيق» (ADR-117). خارج الإنتاج فقط تقبل اللوحة الرمز نفسه في ترويسة `x-nx-operator-token` للسكربتات وللقطات الشاشة.
...
### ٤.١ عنوان الاستقبال، لمن يحتاجه

بعض المزودين لا يجيبون على النداء نفسه. يحدّثون البيانات على جدولهم ثم ينادونك. أصدر لهم عنواناً:

```bash
docker compose run --rm --no-deps api pnpm provision provider:callback \
  --provider lean --environment sandbox \
  --ref kms://providers/lean/sandbox/webhook \
  --header lean-signature --algorithm sha512
```

يطبع عنواناً بالشكل `https://api.<domain>/v1/callbacks/<قيمة معتِمة>`. الصقه في لوحة المزوّد، وضع سر الـwebhook في المخزن تحت المرجع نفسه بالمفتاح `webhookSecret`.

ثلاثة أشياء يفعلها هذا الطريق عمداً:

- **العنوان معتِم ولا يحمل اسم مزوّد.** القاعدة 5 تخص كل سطح عام، ومسار يقول من يخدمنا هو ذلك الاسم منشوراً.
- **التوقيع على البايتات الخام.** بدن غير موقّع يُرفض بـ401 قبل أن يُقرأ، وعنوان مجهول يُرفض بـ404: لا يفرّق بينهما غريب يبحث عن عنوان حي.
- **البدن لا يُخزَّن.** المحفوظ نوع الحدث ومعرّفه وبصمة البايتات. حمولة المزوّد تحمل أرقام حسابات وأسماء، والقاعدة 4 لا تتوقف لأن غيرنا أرسلها.

وإعادة التسليم حدث واحد لا اثنان. المزوّد الذي لا يسمع جواباً سريعاً يعيد الإرسال، والجواب 202 في المرتين.

**التدوير يُبطل العنوان القديم فوراً.** لا تضغط «تدوير العنوان» إلا ولوحة المزوّد مفتوحة أمامك.
...
ثم راجع `/operator/verification/health` و`/operator/pricing`: الأولى تقول هل المزوّد يجيب، والثانية تعرض تكلفة كل منتج وسعره وهامشه وترفض سعراً تحت التكلفة.

---

## اسأل النظام بدل أن تتذكّر

كل ما سبق يفحصه النشر عن نفسه في `/operator/verification/readiness`. الشاشة تقول ما هو مضبوط، وما ينقص، وماذا يُكتب بالضبط لضبطه، وترتّب ما يوقف الإطلاق فوق ما لا يوقفه.

افتحها بعد كل خطوة أعلاه، وقبل أول عميل.
