...
describe('the links in an alert', () => {
  ...
  it('names and rates every event, so none renders as its own code', () => {
    for (const event of WEBHOOK_EVENT_TYPES) {
      expect(WEBHOOK_EVENT_LABELS_AR[event]).toBeTruthy();
      expect(WEBHOOK_EVENT_LABELS_AR[event]).not.toBe(event);
      expect(SEVERITY_LABELS_AR[WEBHOOK_EVENT_SEVERITY[event]]).toBeTruthy();
    }
  });
  ...
  it('offers every event it can send, and sends every event it offers', () => {
    ...
    const offered = [...WEBHOOK_EVENT_TYPES].sort();
    const named = (Object.keys(WEBHOOK_EVENT_LABELS_AR) as WebhookEventType[]).sort();
